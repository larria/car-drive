// 场景0：直角转弯（L 形道路）
// 道路宽 RW = 车辆轴距 + 1.00m（动态，随当前车辆变化）；
// 入口直道长 L1=12000mm，出口直道长 L2=10000mm。
// 坐标 mm。车从入口底部朝上行驶，转弯后向右出口。
//
// 动态参数机制：params(V) 基于车辆参数计算变量，元素坐标用 ${expr} 引用（见 scene-loader）。
...
export const scene0 = {
  ...
  name: '直角转弯',
  scale: 7,
  // 动态参数：道路宽 = 当前车辆轴距 + 1m；L1/L2 为直道长度（一并提供给占位符表达式）
  params: (V) => ({
    RW: V.wheelbase + 1000,
    L1,
    L2,
